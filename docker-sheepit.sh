...
if [ -n "$HOSTNAME" ];          then COMMAND+=" -hostname $HOSTNAME";                    else echo "Using hostname: freezingDaniel_Docker_$(date +%Y-%m-%d_%H-%M-%S)"; COMMAND+=" -hostname freezingDaniel_Docker_$(date +%Y-%m-%d_%H-%M-%S)"; fi
